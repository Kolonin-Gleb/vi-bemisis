...
function Book({ onBack }) {
  const [showBookPlace, setShowBookPlace] = useState(false);

  if (showBookPlace) {
    return <BookPlace onBack={() => setShowBookPlace(false)} />;
  }

  return (
    <div className="book-root">
      <header className="book-header">
        <span className="book-logo">it.am</span>
        <div className="book-user">
          <span className="book-username">Иван Иванов</span>
          <span className="book-usericon">
      ...
      <main className="book-main">
        <button className="book-back" onClick={onBack} aria-label="Назад">
          <span>&#8592;</span>
        </button>
        <h2 className="book-title">Забронировать</h2>
        <div className="book-btns">
          <button className="book-btn" onClick={() => setShowBookPlace(true)}>Рабочее место</button>
          <button className="book-btn" onClick={() => setShowBookPlace(true)}>Приставку</button>
        </div>
      </main>
      <section className="book-progress">
        <div className="book-progress-left">
          <b>Личный прогресс:</b>
          <ul>
        ...
        <div className="book-progress-right">
          <b>Не пропусти!</b>
          <ul>
